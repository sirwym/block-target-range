...
// Phase2 目标 5 把武器：删除 rpg7，新增 ak47/m4a1
// rpg7 的关键 bone 测试单独删除，DEFAULT_HIDDEN_BONE_ROOTS 不依赖 rpg7
const WEAPONS = ["deagle_golden", "m107", "m95", "ak47", "m4a1"];

function loadGeo(weapon) {
  const raw = fs.readFileSync(path.join(GEO_DIR, `${weapon}_geo.json`), "utf8");
...
test("createTaczGeoModel 保留三轴 rotation 不被丢弃", () => {
  ...
  try {
    ...
    // v9 修复 C：bone rotation 改用 rotationQuaternion（ZXY 顺序），不再用 Euler rotation
    ...
    // 验证 ZXY 顺序四元数：q = qY(45°) * qX(30°) * qZ(60°)
    // 四元数乘法 qA*qB 表示先应用 qB 再应用 qA，ZXY 顺序对应 q = qY * qX * qZ
    // v10: 曾尝试 Microsoft 官方文档的 XYZ 顺序但导致 m95 散架，已回退为 ZXY（TaCZ mod 实测）
    ...
    const expected = qY.multiply(qX).multiply(qZ);
...
test("rpg7 geo boneMap 包含关键 bone", () => {
  // Phase2 改用 5 把目标武器（ak47/m4a1 替代 rpg7），但保留 rpg7 关键 bone 测试用于回归。
  // 这里独立 loadGeo("rpg7")，不依赖 WEAPONS 数组。
  const geo = loadGeo("rpg7");
  const { scene, engine } = createScene();
  try {
    const model = createTaczGeoModel(scene, geo, null, { weaponId: "rpg7" });
    const required = ["root", "rocket", "mag_hand", "righthand", "lefthand", "constraint"];
    for (const name of required) {
      assert.ok(model.boneMap.has(name), `boneMap 包含 ${name}`);
    }
  } finally {
...
test("m107 geo boneMap 包含关键 bone", () => {
  const geo = loadGeo("m107");
  const { scene, engine } = createScene();
  try {
    const model = createTaczGeoModel(scene, geo, null, { weaponId: "m107" });
    const required = ["root", "mag_standard", "bolt2", "righthand", "lefthand", "constraint", "gun_barrel"];
    for (const name of required) {
      assert.ok(model.boneMap.has(name), `boneMap 包含 ${name}`);
    }
  } finally {
...
test("M107/M95 第一人称异常漂浮 cube 默认不渲染", () => {
  const hiddenCubes = {
    m107: {
  ...
  for (const [weapon, rules] of Object.entries(hiddenCubes)) {
    const geo = loadGeo(weapon);
    const { scene, engine } = createScene();
    try {
      const model = createTaczGeoModel(scene, geo, null, { weaponId: weapon });
      for (const [boneName, rule] of Object.entries(rules)) {
        assert.ok(model.boneMap.has(boneName), `${weapon} boneMap 保留 ${boneName}`);
        if (rule === true) {
          assert.equal(
            model.cubes.some((cube) => cube.boneName === boneName),
            false,
            `${weapon} 默认不渲染 ${boneName} 整组异常 cube`
          );
        } else {
          for (const cubeIndex of rule) {
            assert.equal(
              model.cubes.some((cube) => cube.boneName === boneName && cube.cubeIndex === cubeIndex),
              false,
              `${weapon} 默认不渲染 ${boneName}-${cubeIndex}`
            );
          }
        }
      }
...
test("无旋转 bone 下旋转 cube pivot local 位置等于 (cubePivot - bonePivot)/16", () => {
  // Phase2 优化：旋转 parent 下也使用 bind-pose 差值作为 local position。
  // 这个测试用无旋转 bone 验证 cubePivotNode local = (cubePivot - bonePivot)/16 基础情形。
  ...
  try {
    ...
    // cubePivotNode.position 应为 (cubePivot - bonePivot) / 16 = ([7,3,1] - [4,2,0]) / 16
    const expectedX = (7 - 4) / 16;
    const expectedY = (3 - 2) / 16;
    ...
    // mesh.position 应为 (cubeCenter - cubePivot) / 16 = ([7,3,1] - [7,3,1]) / 16 = [0,0,0]
    // cubeCenter = origin + size/2 = [6+1, 2+1, 0+1] = [7, 3, 1]
...
const COMPACT_BOUNDS_SPEC = {
  deagle_golden: { maxX: 0.30, maxY: 1.20, maxZ: 1.40 },  // 理论 0.11/0.67/0.81，容差后约 0.3/1.2/1.4
  m107:          { maxX: 1.20, maxY: 3.00, maxZ: 6.50 },  // 理论 0.57/1.68/3.85，容差后约 1.2/3.0/6.5
  m95:           { maxX: 0.90, maxY: 3.20, maxZ: 6.70 },  // 理论 0.37/1.74/3.94，容差后约 0.9/3.2/6.7
  ak47:          { maxX: 2.50, maxY: 2.50, maxZ: 5.00 },  // 宽松防散架上限，待视觉验收后收紧
  m4a1:          { maxX: 3.00, maxY: 3.50, maxZ: 5.00 },  // fix 后 X=2.513/Y=3.076，放宽防正常尺寸误报
};
...
const KEY_CUBES = {
  ...
  m107: { boneName: "l2", cubeIndex: 0 },
  m95: { boneName: "head3", cubeIndex: 0 },
  ak47: { boneName: "wood", cubeIndex: 0 },
  m4a1: { boneName: "group3", cubeIndex: 0 },
};
...
test("Phase2 失败基线中的主结构 outlier 不允许被 profile 直接隐藏", () => {
  const cases = [
    ...
    { weaponId: "m107", geoName: "m107", mainBones: ["bone"] },
    { weaponId: "m95", geoName: "m95", mainBones: ["bone"] },
    { weaponId: "ak47", geoName: "ak47", mainBones: ["wood4"] },
    { weaponId: "m4", geoName: "m4a1", mainBones: ["bone70", "octagon4", "bone", "bone2"] },
  ];
    ...
    try {
      ...
      for (const [boneName, expectedParent] of Object.entries(KEY_BONE_PARENTS[weapon])) {
        ...
        assertArrayClose(
          ...
          localDiff(boneDebug.originalPivot, boneDebug.parentPivot),
          `${weapon}.${boneName} localPosition = pivot-parentPivot`
        );
      }
    } finally {
      engine.dispose();
...
test("旋转 parent 下 cube pivot 仍使用 bind-pose 差值作为 local position", () => {
  // 验证 cubePivotNode local position 不依赖 parent rotation，
  // 始终等于 (cube.pivot - bone.pivot) * PIXEL_TO_UNIT。
  ...
  try {
    ...
    // cubePivotNode local position = (cubePivot - bonePivot) * PIXEL_TO_UNIT
    // = ([9,1,1] - [4,0,0]) / 16 = [5, 1, 1] / 16
    ...
    // cubeCenter = origin + size/2 = [8+1, 0+1, 0+1] = [9, 1, 1]
    // cubePivot = [9, 1, 1]
    // mesh local = [0, 0, 0]
    const mesh = model.cubes[0].mesh;
    ...
    assert.ok(Math.abs(mesh.position.y) < 0.0001, `mesh.position.y: ${mesh.position.y} 期望 0`);
...
// ===== v10 新增：旋转顺序 ZXY 正确性测试 ====
// 验证修复 C：bone rotation 使用 Quaternion ZXY 顺序（qY * qX * qZ）
// 现有 2 个带旋转 bone 测试用单轴 Y 旋转，单轴旋转不受顺序影响，
// 必须用三轴旋转才能验证 ZXY 顺序与 Babylon 默认 YXZ 顺序的不同
// v10: 曾尝试 Microsoft 官方文档的 XYZ 顺序但导致 m95 散架，已回退为 ZXY（TaCZ mod 实测）

test("createTaczGeoModel 旋转顺序为 ZXY（先 Z 后 X 再 Y）", () => {
  // 构造三轴旋转 bone，验证 rotationQuaternion 等于 qY * qX * qZ
  // 单轴旋转不受顺序影响，必须用三轴旋转才能验证 ZXY 顺序
  const mockGeo = {
    format_version: "1.12.0",
    "minecraft:geometry": [{
      description: { identifier: "geometry.zxy_test", texture_width: 16, texture_height: 16 },
      bones: [{
        name: "root",
        pivot: [0, 0, 0],
        rotation: [30, 45, 60], // 三轴非零，XYZ 顺序不同结果不同
        cubes: [{ origin: [0, 0, 0], size: [1, 1, 1], uv: [0, 0] }],
      }],
    }],
  };
  const { scene, engine } = createScene();
  try {
    const model = createTaczGeoModel(scene, mockGeo, null, { weaponId: "zxy-test" });
    const rootNode = model.boneMap.get("root");
    assert.ok(rootNode.rotationQuaternion, "rotationQuaternion 非空（修复 C 生效）");
    // 构造期望四元数：qY * qX * qZ（ZXY 顺序，四元数乘法 qA*qB 表示先应用 qB 再应用 qA）
    const DEG = Math.PI / 180;
    const qZ = BABYLON.Quaternion.RotationAxis(BABYLON.Axis.Z, 60 * DEG);
    const qX = BABYLON.Quaternion.RotationAxis(BABYLON.Axis.X, 30 * DEG);
    const qY = BABYLON.Quaternion.RotationAxis(BABYLON.Axis.Y, 45 * DEG);
...
test("cube.pivot 缺失时 fallback 到 cube 自身中心（origin + size/2）", () => {
  // 构造有 rotation 无 pivot 的 cube，验证 cubePivotNode 世界坐标 = cube 中心
  ...
  try {
    ...
    const pivotNode = children.find(c => c.getClassName() === "TransformNode");
    assert.ok(pivotNode, "cubePivotNode 存在");
    // 期望 cubePivotNode 本地 position = cube 中心 = origin + size/2 = [5, 3, 1] / 16
    // 用本地 position 而非世界坐标，因为 createTaczGeoModel 末尾的居中逻辑
    // （centeringNode）会偏移顶层 bone 的世界坐标，但 cubePivotNode.position 不受影响
...
test("不传 visibilityProfile 时走旧 DEFAULT_HIDDEN_BONE_ROOTS fallback", () => {
  // m107 不传 profile，应该仍走 DEFAULT_HIDDEN_BONE_ROOTS.m107
  const geo = loadGeo("m107");
  const { scene, engine } = createScene();
  try {
    const model = createTaczGeoModel(scene, geo, null, { weaponId: "m107" });
    // sight_folded 应该被隐藏（来自 DEFAULT_HIDDEN_BONE_ROOTS.m107）
...
test("传入 visibilityProfile + 旧 fallback 同时生效（合并）", () => {
  // m107 传入额外 profile.defaultHiddenBones，应该与旧 DEFAULT_HIDDEN_BONE_ROOTS 合并
  const geo = loadGeo("m107");
  const { scene, engine } = createScene();
  try {
    const profile = {
